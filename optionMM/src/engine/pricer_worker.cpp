...
void TradingEngine::pricer_loop() noexcept {
    ...
    while (!stop_flag_.load(std::memory_order_relaxed)) {
        ...
        const double future_tick_size = instruments_[future_id].tick_size;
        const ProductPricingConfig pricing = product_pricing(prod);
        const double F_mid = apply_base_offset(F_mid_raw, future_tick_size, pricing);
        double F_bid = apply_base_offset(F_bid_raw, future_tick_size, pricing);
        double F_ask = apply_base_offset(F_ask_raw, future_tick_size, pricing);
        if (F_mid < 1e-10) {
            pending_product[prod] = false;
            next_option_offset[prod] = 0;
            continue;
